// src/main.js
// One tiny module to boot, wire buttons, draw, and show diagnostics.

import { createGameState, update, tryDeployAt } from './logic.js';

// ---------- Diagnostics (tiny + always visible unless toggled) ----------
function initDiag() {
  ...
  if (!box) {
    ...
    box.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
        ...
        <span id="lc-diag-ver" style="opacity:.75"></span>
      </div>
    ...
    // robust key handler (capture so nothing can swallow it)
    const keyHandler = (e) => {
  ...
  const api = {
    setVersion: (v)=> ver.textContent = `v ${v}`,
    line: (prefix, msg) => {
...
// ---------- UI wiring + loop ----------
async function start(version='0.1.4'){
  const diag = initDiag(); diag.setVersion(version);
  diag.step('boot: main.js');

  const canvas = $('game');
  if (!canvas) { diag.err('Missing <canvas id="game">'); return; }

  // set canvas size (target 900h)
  function resize(){
  ...
  // screens (works even if some are missing)
  const scrMenu = $('screenMenu'), scrPlay = $('screenPlay'),
        scrEnc = $('screenEncyclopedia'), scrLog = $('screenUpdateLog');
  const btnPlay = $('btnPlay'), btnEnc = $('btnEncyclopedia'),
        btnLog = $('btnUpdateLog'), backP = $('btnBackFromPlay'),
        backE = $('btnBackFromEnc') || $('btnBackFromEncyclopedia'), backL = $('btnBackFromLog');

  const toMenu = ()=>{ show(scrMenu); hide(scrPlay); hide(scrEnc); hide(scrLog); canvas.style.pointerEvents='none'; };
  const toPlay = ()=>{ hide(scrMenu); show(scrPlay); hide(scrEnc); hide(scrLog); canvas.style.pointerEvents='auto'; };
  const toEnc  = ()=>{ hide(scrMenu); hide(scrPlay); show(scrEnc); hide(scrLog); canvas.style.pointerEvents='none'; };
  const toLog  = ()=>{ hide(scrMenu); hide(scrPlay); hide(scrEnc); show(scrLog); canvas.style.pointerEvents='none'; };

  // start on menu if present, otherwise play
  (scrMenu && scrPlay) ? toMenu() : toPlay();
  ...
  // select cards with 1/2, Esc cancels
  const setSel = (s)=>{ state.selectedHandSlot = s; state.showPlacementOverlay = (s!==null); };
  ...
  canvas.addEventListener('click',(ev)=>{
    if (scrPlay && scrPlay.style.display==='none') return; // only while playing
  ...
  // safe RAF loop (never dies silently)
  let last = performance.now();
  function tick(now){
    try{
      const dt = Math.max(0.001, Math.min(0.05, (now-last)/1000)); last = now;
      if (!scrPlay || scrPlay.style.display !== 'none'){ // only run while Play is shown
        update(state, dt);
        draw(state);
      }
    }catch(e){
      console.error(e);
      diag.err(e?.stack || e?.message || String(e));
    }
